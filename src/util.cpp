...
const char * const GLOBALBITCOIN_CONF_FILENAME = "globalbitcoin.conf";
const char * const GLOBALBITCOIN_PID_FILENAME = "globalbitcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "globalbitcoind -foo=bar
...
#else
    const char* pszModule = "globalbitcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GlobalBitcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GlobalBitcoin
    // Mac: ~/Library/Application Support/GlobalBitcoin
    // Unix: ~/.globalbitcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GlobalBitcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GlobalBitcoin";
#else
    // Unix
    return pathRet / ".globalbitcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", GLOBALBITCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", GLOBALBITCOIN_PID_FILENAME)));
}
